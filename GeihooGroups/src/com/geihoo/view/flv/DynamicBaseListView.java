...
import com.geihoo.utils.AppUtil;
import com.geihoo.view.flv.DynamicListView.OnScrollListener;
import com.geihoo.view.flv.DynamicListView.OnScrollStateListener;
import com.geihoo.view.flv.DynamicListView.OnStartListener;

public abstract class DynamicBaseListView extends DynamicAdapterView<ListAdapter>
	...
	public DynamicBaseListView(Context context) {
		super(context);
		initAbsListView();
	}

	public DynamicBaseListView(Context context, AttributeSet attrs) {
		this(context, attrs, android.R.attr.absListViewStyle);
	}

	public DynamicBaseListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initAbsListView();
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.DynamicBaseListView, defStyle, 0);
		Drawable d = a
				.getDrawable(R.styleable.DynamicBaseListView_android_listSelector);
		if (d != null) {
			setSelector(d);
		}
		mDrawSelectorOnTop = a.getBoolean(
				R.styleable.DynamicBaseListView_android_drawSelectorOnTop, false);
		boolean scrollingCacheEnabled = a.getBoolean(
				R.styleable.DynamicBaseListView_android_scrollingCache, true);
		setScrollingCacheEnabled(scrollingCacheEnabled);
		int transcriptMode = a.getInt(
				R.styleable.DynamicBaseListView_android_transcriptMode,
				TRANSCRIPT_MODE_DISABLED);
		setTranscriptMode(transcriptMode);
		int color = a.getColor(
				R.styleable.DynamicBaseListView_android_cacheColorHint, 0);
		setCacheColorHint(color);
		boolean smoothScrollbar = a.getBoolean(
				R.styleable.DynamicBaseListView_android_smoothScrollbar, true);
	...
	public void jumpDrawablesToCurrentState() {
		...
		if (AppUtil.isSupport(11) && mSelector != null) {
	...
	boolean performLongPress(final View child, final int longPressPosition,
		...
		if (mOnItemLongClickListener != null) {
			handled = mOnItemLongClickListener.onItemLongClick(
					(DynamicListView) this, child, longPressPosition, longPressId);
		}
		if (handled) {
	...
	private class FlingRunnable implements Runnable {
		...
		void start(int initialVelocity) {
			...
			mTouchMode = TOUCH_MODE_FLING;
			ViewCompat.postOnAnimation(DynamicBaseListView.this, this);
		}
	...
	class AdapterDataSetObserver extends
			DynamicAdapterView<ListAdapter>.AdapterDataSetObserver {
		@Override
