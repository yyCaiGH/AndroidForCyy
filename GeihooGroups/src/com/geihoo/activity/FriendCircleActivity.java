...
public class FriendCircleActivity extends BaseActivity {
	...
	private FriendCircleBean mFc;// 朋友圈

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		this.setContentView(R.layout.fragment_society_main);
		mFc = this.getIntent().getParcelableExtra("friendCircle");
		initView();
		initData();
	...
	protected void initView() {
		...
		if(mFc!=null){
			societyNameSus.setText(mFc.getName());
		}
		topSuspension = (RelativeLayout) this
				.findViewById(R.id.rl_top_suspension);

		susTopPosted = (LinearLayout) this.findViewById(R.id.btn_group_layout);

		TextView postMessage = (TextView) this
				.findViewById(R.id.send_message_btn);
	...
	private void initPostListView() {
		...
		View v = LayoutInflater.from(this).inflate(R.layout.view_zuzu_cover,
				null);
		...
		postPic.setOnClickListener(this);
		TextView tv_stick_posts = (TextView) v
				.findViewById(R.id.tv_stick_posts);
		tv_stick_posts.setVisibility(View.GONE);
		societyName = (TextView) v.findViewById(R.id.tv_zz_name);
		if(mFc!=null){
			societyName.setText(mFc.getName());
		}
		...
		if (mFc != null) {
			if (mFc.getBgIcon() == null) {
				zuzubg.setImageBitmap(ImageUtil.readBitMap(this,
						R.drawable.zz_def_bg));
			} else {
				zuzubg.setImageBitmap(mFc.getBgIcon());
			}
		}
		// 设置头像
		...
		if (mFc != null) {
			if (mFc.getHeadIcon() == null) {
				zuzuHeadImg.setPic(ImageUtil.readBitMap(this,
						R.drawable.zz_def_head));
			} else {
				zuzuHeadImg.setPic(mFc.getHeadIcon());
			}
		}
		...
		postListView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				if (y < 0) {
					...
					topSuspension.setBackgroundColor(Color
							.parseColor("#deffffff"));
				...
				if (topPostedY < 0) {
					...
					susTopPosted.setBackgroundColor(Color
							.parseColor("#deffffff"));
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case SET_POSTLIST:
				societyPostAdapter = new SocietyPostAdapter(
						FriendCircleActivity.this, postList,
						SocietyPostAdapter.ZZ_DYNAMIC);
