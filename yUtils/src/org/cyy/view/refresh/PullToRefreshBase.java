package org.cyy.view.refresh;

import org.cyy.view.refresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    /**
     * 判断是否在顶部还是在底部，而决定是否把事件给onTouchEvent来实现滚动效果
     */
    @Override
        ...
        case MotionEvent.ACTION_UP:
            if (mIsHandledTouchEvent) {
                ...
                if (isReadyForPullDown()) {
                    // 调用刷新
                    if (mPullRefreshEnabled && (mPullDownState == State.RELEASE_TO_REFRESH)) {//可刷新状态
                        startRefreshing();
                        handled = true;
                    }
                    resetHeaderLayout();
    ...
    public void setPullLoadEnabled(boolean pullLoadEnabled) {
        ...
        mScrollLoadEnabled = !pullLoadEnabled;//要么滑到底部自动加载，要么上拉加载@cyy
    }
    
    @Override
    public void setScrollLoadEnabled(boolean scrollLoadEnabled) {
        mScrollLoadEnabled = scrollLoadEnabled;
        mPullLoadEnabled = !scrollLoadEnabled;//要么滑到底部自动加载，要么上拉加载@cyy
    }
...
//            onStateChanged(mPullUpState, false);
    ...
    protected void resetHeaderLayout() {
        ...
        if (refreshing && scrollY <= mHeaderHeight) {//虽然正在刷新，但用户自己上拉，不看刷新效果
            smoothScrollTo(0);
            return;
        }
        
        if (refreshing) {//正在刷新，显示刷新效果，不隐藏头部
